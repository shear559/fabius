...
| **R5** | Ground dependent Actions in prior real Observations. Choose a fixed integer window w≥1; after w completed no-progress cycles the next control step is Replan or an authorized Terminal. These are safety guards under the stated prefix/terminal semantics. Mandatory enforcement is a policy choice; an indicator objective can represent the feasible set. | agent control flow — the per-step gate (vs disciplina's terminal prove-gate) | **heuristic** (qualitative; a control-flow invariant, not an optimization) |
...
| **M4** | One-step lookahead: ΔV_k=p_k·V_fix−c_retry; continue iff positive, stop at equality. Under assumed g_k=g_1ρ^(k−1), 0<ρ<1: first rejected retry k*=1 if g_1≤c_retry, otherwise 1+ceil(log(g_1/c_retry)/log(1/ρ)); at most k*−1 retries precede it. Repeated-cause stopping and a ~3-retry budget are separate heuristics. | stochastic processes / decision theory — optimal stopping (myopic one-step lookahead); reflection as a policy update | **real-math** |
