...
Version **3.1.0** adds reference doctrine — confirmation by action class for agents that act through a screen or channel, the standing-job and live-voice shapes, the visual-system template, personal-agent trust boundaries and the grant lifecycle, the connected-service terms gate, prove-at-the-surface verdicts, declarative preference records — plus pointer sentences in six sealed contracts. No helper code changed and no new host acceptance run was performed for it: the 3.0.x receipts describe the installation mechanism, not this release's contract text. Five additional maintainer smoke scenarios were run against the 3.1.0 references; their receipt is qualitative evidence, not a benchmark.

Version **3.2.0** adds reference doctrine under the existing fifteen owners — capture integrity and anchored meeting records, citation gates, book-length source distillation, the prose tell lint and the existing-site SEO audit method, a reporting shape for a reader who must act, wiring a judge's score into software, the hosted-model tier, audit closure with a cleared-surface record, the likeness and voice consent gate, and the generated-media request — plus pointer sentences in seven sealed contracts and rows in `CORPUS.md` and `ARCHITECTURE.md`. No helper code changed, no skill was added, and no new host acceptance run was performed for it: the 3.0.x receipts describe the installation mechanism, not this release's contract text. Nine additional maintainer smoke scenarios were run against the 3.2.0 references. The first round produced six passes, two partials and one fail; two reference rules and three rubric rows were corrected, the affected cases were rerun in fresh contexts, and all nine then passed. Both rounds are archived in the receipt, which is qualitative evidence, not a benchmark.
