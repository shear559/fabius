...
Informed by **system_prompts_leaks** (asgeirtj, CC0-1.0 compilation; the collected vendor prompts remain their vendors' text) — studied for the confirmation-class split of acting agents (hand-off · confirm-at-action · pre-approved · never-ask), the transmission and credential-around-the-agent rules, the exact-argument and scoped-grant approval regime, the closed act/ask/hand-off/ignore verdict, and the bounded-ask shape, re-expressed in fabius's own voice; no prompt text carried, nothing bundled. See credits/README.md.
