...
- **`fabius-decor`** — the design layer: token vocabulary, the one-accent laws, mobile-first, the live-verify checklist — **and the data-visualization concern** (the *figura* library: data-ink charts, reproducible tokenized SVG, diagrams-as-code). A brand reference is captured into the visual-system template before building.
- **`fabius-cohors`** — the agent-engineering layer: the definition schema, least-privilege permissions, the five orchestration patterns (sequential / parallel / hierarchical / human-in-the-loop / swarm). An acting agent confirms by action class (hand-off · confirm-at-action · pre-approved · never-ask); a standing-job agent and a live-voice front-end keep the ruled loop as the brain.
...
- **`fabius-praesidium`** — the defensive-security layer: STRIDE threat-modeling per trust boundary, the OWASP pass, secrets + least-privilege hygiene, and a severity→fix→proof finding contract; the same grid covers a personal agent's boundaries (command channel, connected accounts, outward actions, derived store, acting machine, vendor terms). Hardens, never weaponizes.
...
Architecture planning and assessment load [`architecture-decisions.md`](skills/fabius-disciplina/references/architecture-decisions.md); updates load [`transactional-updates.md`](skills/fabius-disciplina/references/transactional-updates.md). Skill refinement loads the router's [`skill-maintenance.md`](skills/fabius/references/skill-maintenance.md). Legacy record consolidation loads Archivum's [`memory-migration.md`](skills/fabius-archivum/references/memory-migration.md). These are on-demand procedures, not additional discoverable skills or installed services. Delegation uses the current harness's actual tool schema; the local model runner performs one loop. The separate original Cohors scheduler executes a validated dependency graph through an explicitly supplied, caller-authorized runner; it does not create model sessions or a sandbox on its own.

## The spine

- **`references/` and `scripts/`** — original on-demand Fabius procedures and helpers under their existing owners. The imported example trees and nested reference contracts were removed in 3.0.0. Cohors provides agent contracts and dependency scheduling; Archivum provides bounded local lexical retrieval with source citations; Decor provides semantic tokens, original layout recipes and deterministic scenes; Disciplina provides a plan/evidence consistency ledger. [CORPUS.md](CORPUS.md) is the entry index, [credits/capabilities.json](credits/capabilities.json) maps implementations to tests, and [credits/retired-upstream.json](credits/retired-upstream.json) preserves the old source registry. None of these helpers claims whole-framework parity with retired upstream projects.
...
- **Target a new tool** — follow the [qualified portable-core guidance](README.md#install-in-your-agent-app), obtain permission where required by the licence, preserve existing instructions and test the actual host.
